...
export const DashboardWithDynamicDataWithPropsPage = ({
  statistics,
}: {
  statistics: TableData[];
}) => {
  const [tableData, setTableData] = useState<TableData[]>(statistics ?? []);
  const [isLoading, setIsLoading] = useState(statistics ? false : true);

  useEffect(() => {
    const fetchStatistics = async () => {
      try {
        const response = await fetch("http://localhost:5432/api/statistics");
        if (!response.ok) {
      ...
      <div className="flex flex-1 h-full overflow-y-auto flex-col p-6 gap-6">
        ...
        <div className="blink-surface-default border blink-border-container-white rounded-lg p-4">
          ...
          <div className="w-full overflow-auto">
            {isLoading ? <TableSkeleton /> : renderTable(tableData)}
          </div>
        </div>
